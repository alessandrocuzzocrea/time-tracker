<script lang="ts">
  ...
  onMount(async () => {
    ...
    startTimeString = formatDateTime(startTime);
    endTimeString = formatDateTime(endTime);
  });

  function saveNew() {
    TimeEntryStore.add2(taskId, description, startDate, endDate);
...
<div class="flex flex-auto flex-col space-y-4">
  <p class="text-2xl">NEW TIME ENTRY taskId</p>
